...
class DataFrameManager:
    """
    DataFrameManager class for managing multiple DataFrame objects that originate, or are somehow related to, the same data source (e.g. a single Excel file with multiple sheets), functioning similarly to a dictionary of dataframes with additional functionality.
    """
    ...
    def get_sheet_titles(self, df_dict: dict, drop_title_sheet: bool = False) -> dict:
        ...
        for _, (table, table_name) in df_dict["Titles"].dropna().iterrows():
            if "Table" in table and table != "Table":
                ...
                table_name = clean_string_with_patterns(table_name, "glottal_stop")
